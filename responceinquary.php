...
$inquiry_list='';
//get inqury details
 $query="SELECT i.*,s.staffName FROM inquiry as i left outer join staff as s on s.staffID=i.responceStaffID;";
 $inquiry=mysqli_query($conp,$query);

 //fill inquiry details to table
 
 if($inquiry){
     while($inquiry1=mysqli_fetch_assoc($inquiry)){
 $insquiryId=$inquiry1['inquaryID'];
 $userID=$_SESSION['adminid'];
         $inquiry_list.="<tr>";
         $inquiry_list.="<td>{$inquiry1['inquaryName']}</td>";
         $inquiry_list.="<td>{$inquiry1['email']}</td>";
         $inquiry_list.="<td>{$inquiry1['massage']}</td>";
         if($inquiry1["responceStaffID"]>0){
            $inquiry_list.="<td> <div class='form-check form-switch'><input class='form-check-input' onclick='saveResponder($userID,$insquiryId)' name='responce' type='checkbox' id='flexSwkDefault' checked disabled></div></td>";
         }else{   
            $inquiry_list.="<td> <div class='form-check form-switch'><input class='form-check-input' onclick='saveResponder($userID,$insquiryId)' name='responce' type='checkbox' id='flexSwitchCheckDefault'></div></td>";
         }
         $inquiry_list.="<td>{$inquiry1['staffName']}</td>";
         $inquiry_list.="</tr>";
                        ...
                        <table class="table bg-white rounded shadow-sm  table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" width="10">Responce</th>
                                    <th>Responce Person</th>
                                </tr>
                            </thead>
        ...
        if(window.XMLHttpRequest){
             ...
             xmlhttp.onreadystatechange= function(){
                 if(this.readyState==4 && this.status==200){
                    window.location.href="responceinquary.php";
                     
                 }
                 };
                  xmlhttp.open("GET","saveResponder.php?p="+userID+"_"+inqID,true);
                  xmlhttp.send();
    }
</script>
</body>
