...
$jobs_list='';
 $query="select b.bookingID, s.serviceName, b.Date, p.name, dt.dayTypeName, b.numberOfGuest, pc.packageName 
 from booking as b, bookingprice as bp , service as s, daytype as dt,place as p, package as pc
  where b.bookingID=bp.bookingID and s.serviceID=bp.serviceID and p.placeID=b.placeID and b.dayTypeID=dt.dayTypeID
   and pc.packageID=bp.packageID and bp.regiProviderID=$pID and bp.status=2;";

 $jobs=mysqli_query($conp,$query);

 $query="select images from regiprovider where regiproviderID='$pID'";
...
//  $bookingID=$jobs1['bookingID'];
 $pID=$_SESSION['regiProviderID'];
        ...
        //  $jobs_list.="<td>{$jobs1['name']}</td>";
        //  if($jobs1["regiProviderID"]>0){
        //     $jobs1.="<td><button onclick='saveResponder($pID,$bookingID)' name='approve'disabled>Approve</button></td>";
        //  }else{
        //     $jobs_list.="<td><button onclick='saveResponder($pID,$bookingID)' name='approve'>Approve</button></td>";
        //  }
        //  if($jobs1["regiProviderID"]>0){
        //     $jobs1.="<td><button onclick='saveResponder($pID,$bookingID)' name='approve'disabled>Reject</button></td>";
        //  }else{
        //     $jobs_list.="<td><button onclick='saveResponder($pID,$bookingID)' name='approve'>Reject</button></td>";
        //  }
        //  $jobs_list.="<td>{$jobs1['status']}</td>";
         $jobs_list.="</tr>";
...
<body>
  ...
  <main style="margin-top: 58px ">
    <div class="container pt-4">
      <!-- Section: Main chart -->
      <section class="mb-4 row my-0">
        <div class="card">
          <div class="card-header py-3">
            <h5 class="mb-0 text-center"><strong>Approved orders</strong></h5>
          </div>
          <div class="row">
