...
//check database
$staff_list='';
$query="SELECT * FROM staff WHERE is_staffmember_delete=0 ORDER BY staffName";
...
if($staff){
    while($staff1=mysqli_fetch_assoc($staff)){
        ...
        $staff_list.="<tr>";
        $staff_list.="<td>{$staff1['staffID']}</td>";
        $staff_list.="<td>{$staff1['staffName']}</td>";
        $staff_list.="<td>{$staff1['staffEmail']}</td>";
        $staff_list.="<td><a href=\"modify-admindashboard.php?id={$staff1['staffID']}\">Edit</a></td>";
        $staff_list.="<td><a href=\"delete-member.php?id={$staff1['staffID']}\">Delete</a></td>";
        $staff_list.="</tr>";
    }
 }
 else{
        ...
        <div id="page-content-wrapper">
            <nav class="navbar navbar-expand-lg navbar-light bg-transparent py-4 px-4">
                ...
                <div class="card-body py-1 px-md-6">
                   <?php
                   $sql="select rp.providerName , Round(avg(p.rate),1) as avRate from providerrate as p, regiprovider as rp where rp.regiProviderID=p.providerID group by p.providerID order by avRate desc;";
                   $rs=mysqli_query($conp,$sql);
                   $sd=array(array('providerName','rate'));
                   while($row=mysqli_fetch_assoc($rs)){
                        array_push($sd,array($row['providerName'],(float)$row['avRate']));
                   }
                  // $sd=array(array('d','ds'),array('p1',5),array('p12',4),array('p3',6),array('p4',5));
                   ?>
